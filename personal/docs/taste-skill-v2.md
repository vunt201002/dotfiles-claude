<!-- VENDORED — do not edit by hand. Re-vendor from source to update. -->

# taste-skill v2 (vendored reference)

> **Nguồn:** https://github.com/Leonxlnx/taste-skill `skills/taste-skill/SKILL.md`
> **License:** MIT, © 2026 Leonxlnx. Giữ nguyên bản gốc, không sửa nội dung.
> **Fetch:** 2026-08-05, commit `e988add20dab`

## Vì sao file này nằm trong `personal/docs/` chứ không phải `personal/skills/`

Cố ý. File trong `docs/` **không** được symlink vào `~/.claude/skills/` và **không** có
`description:` frontmatter, nên nó không bao giờ tự trigger. Nó chỉ được đọc khi có ai đó
chủ động mở, cụ thể là **builder agent** trong vòng builder+judge (xem rule trong
`personal/global-CLAUDE.md`).

Lý do phải chặn auto-trigger: skill này tự khai scope ở dòng đầu là *landing page,
portfolio, redesign — KHÔNG phải dashboard, data table, product UI nhiều bước*, và default
stack của nó là React/Tailwind/shadcn. Joy và Wishlist là Polaris admin + widget nhúng theme
merchant. Để nó tự nhảy vào lúc sửa Joy là đưa chuẩn sai surface.

**Judge KHÔNG đọc file này.** Judge chấm dimension anti-slop bằng `design-eye §E`, vốn đã
chắt tiêu chí thành thang 0-10. File này (~22K token) chỉ dành cho builder lúc generate.
