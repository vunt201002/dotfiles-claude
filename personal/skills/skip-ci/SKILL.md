---
name: skip-ci
description: Push the current branch to remote WITHOUT triggering a CI pipeline, by adding an empty commit whose message carries the real skip keyword ([skip ci]) that GitLab/GitHub actually honors. Use when the local work is a refactor, a comment/format cleanup, a merge-master sync, or anything that does not need to be built or deployed, and you only want the code on remote without burning pipeline minutes. Verifies the repo's CI actually honors the keyword (checks for custom rules that would force a run anyway), refuses to push on master/main, and reports the branch + commit hash. NOT for triggering a deploy (use /trigger-deploy for that — it is the exact opposite). NOT a commit-your-changes skill (use /my-commit for real code commits) — this only ever creates an EMPTY commit and never stages files. Use when asked to "skip ci", "skip-ci", "push không deploy", "push mà đừng build", "đẩy code lên nhưng khỏi chạy pipeline", "/skip-ci", or after a refactor that just needs to land on remote.
---
