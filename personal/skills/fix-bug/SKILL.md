---
name: fix-bug
description: Fix ONE bug inline in this session via Workflow B — prove root cause with runtime evidence before fixing, red-team it, minimal fix, verify + blast radius, ratchet, stop before commit. Kept inline so Workflow B's human checkpoints stay interactive instead of buried in an agent transcript. NOT for a batch — several bugs in context means /fix-bugs-parallel. NOT auto-retry — /fix-bug-loop retries when verify fails. Never commits or pushes. Use when asked "fix this bug", "sửa bug này", "áp workflow B cho bug này", "/fix-bug", or right after pulling up a single bug the user wants fixed now.
---
