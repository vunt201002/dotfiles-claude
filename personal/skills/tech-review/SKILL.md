---
name: tech-review
description: Report-only code-QUALITY review of the current diff (or a named file/dir) — function shape, naming, input/output contract, refactor opportunities. Also flags wrong-branch merges (a feature merged into a feature instead of master); pass a commit hash to audit one merge after the fact. NOT a bug/regression hunt (use /impact-review). NOT an auto-fixer (use /simplify). NOT the pre-landing safety pipeline (use /review). Use when asked "review kỹ thuật", "code này sạch chưa", "có chỗ nào refactor được không", "merge nhầm branch", "commit này merge từ đâu", "tech review", "/tech-review".
---
