---
name: trigger-deploy
description: Re-run a branch's CI pipeline by pushing one empty commit — nothing else, no file edits. For a branch ALREADY configured to deploy (staging ref set, CI rules match): a flaky or cancelled pipeline, a re-deploy after someone else's push, a redeploy with no code change. Warns if the branch matches no deploy job; refuses to push on master/main. NOT for setting up the staging ref (use /deploy-staging). NOT for cutting a release tag (/deploy-tag). NOT for skipping CI (/skip-ci — the opposite). Use when asked "trigger deploy", "deploy lại", "chạy lại pipeline", "empty commit để deploy", "kích pipeline", "/trigger-deploy".
---
