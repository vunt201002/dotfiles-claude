---
name: merge-master
description: Merge master into the current branch and resolve conflicts carefully, split by file type — translation/locale JSON keeps both sides' keys (same-key-different-value still stops and asks), everything else gets context-aware resolution where only additive conflicts auto-apply and anything deeper is proposed before touching. Stops at git add (staged, not committed) so the user reviews. NOT for combining several feature branches — that's /merge-branch. Use when asked "merge master", "merge master vào branch", "resolve conflict", "giải quyết conflict", "branch bị conflict", "/merge-master", or before opening an MR that shows conflicts with the base branch.
---
