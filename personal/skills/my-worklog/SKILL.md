---
name: my-worklog
description: Cross-project daily worklog for many tasks in flight at once — save each task's state keyed by branch at end of day (what's done, what's left, the one next action, what was half-finished mid-edit, plus the running session log carried forward), then next morning see a standup of everything in progress and resume any task with full context reloaded. NOT a quick task checklist — /todo is that, and saves no context. Use when asked "save my work", "what was I doing", "morning standup", "where did I leave off", "resume task", "worklog", or at the start or end of a work day.
---
