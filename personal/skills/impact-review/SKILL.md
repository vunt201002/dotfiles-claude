---
name: impact-review
description: Report-only feature-impact review — reads a diff or MR/PR change plus the surrounding code it touches (callers, dependents, related tests/config) and predicts whether the change breaks existing behavior or introduces a regression. Reasons about consequences, not just the lines that changed. NOT a code-quality/cleanliness review (use /tech-review for that — naming, function shape, refactor opportunities). NOT the global /code-review (that's diff-scoped correctness + quality combined at configurable effort; this skill is feature-behavior-scoped and always reads beyond the diff into callers/dependents). NOT the heavy pre-landing pipeline (use /review for SQL/security/concurrency). NOT a fixer — reports risks, never edits code. Use when asked "review tính năng", "code này có gây lỗi gì không", "review MR", "cái này có break gì không", "dự đoán ảnh hưởng", "impact review", "/impact-review", or before merging a change you're not fully sure is safe.
---
