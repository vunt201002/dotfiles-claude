---
name: deploy-tag
description: Cut the next deploy tag on a GitLab repo after an MR is merged — infers the tag pattern from the repo's own existing tags (never assumes a format), works out the next one, and PRINTS the git tag + push commands for YOU to run. Never runs them, never touches the base branch. Asks which tag to base on when recent tags are inconsistent. Use when asked "deploy tag", "tag tiếp theo", "cut a release tag", "tạo tag deploy", "next deploy tag", "/deploy-tag", or right after merging an MR that needs a release tag.
---
